import { _SERVICE, GetPoolMetricsArgs, Pool, PoolMetrics } from "../../idl/pool_stats";
...
export class PoolStatsService {
    ...
    public async get_pool_metrics(metricsRequest: GetPoolMetricsArgs[]): Promise<Array<PoolMetrics>> {
        const anonymousActor = await getAnonActor<_SERVICE>(
            poolsDataCanister,
            idlFactory
        );
